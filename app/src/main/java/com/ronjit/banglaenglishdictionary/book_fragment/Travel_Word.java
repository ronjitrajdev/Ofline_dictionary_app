...
import com.ronjit.banglaenglishdictionary.book_database.TravelWord_Database;

import java.util.ArrayList;
import java.util.HashMap;


public class Travel_Word extends Fragment {
    private RecyclerView recyclerView_Travel;
    ArrayList<HashMap<String,String> > arrayList = new ArrayList<>();
    HashMap<String,String> hashMap;
    private myTravelAdapter adapter;
    private TravelWord_Database database;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View myView = inflater.inflate(R.layout.fragment_travel__word, container, false);
        recyclerView_Travel = myView.findViewById(R.id.recyclerView_Travel);
        adapter = new myTravelAdapter();
        recyclerView_Travel.setAdapter(adapter);
        recyclerView_Travel.setLayoutManager(new LinearLayoutManager(requireContext()));
        database = new TravelWord_Database(requireContext());
        loadEnglishData(database.getAllData());
        return myView;
    } //===============================
    private class myTravelAdapter extends RecyclerView.Adapter<myTravelAdapter.myViewHolder>{
        ...
        public void onBindViewHolder(@NonNull myViewHolder holder, int position) {
            ...
            String daily_word = hashMap.get("travel_word");
        ...
        if (cursor != null) {
            if (cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    hashMap.put("travel_word", cursor.getString(1));
